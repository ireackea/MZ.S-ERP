...
const FormulationForm: React.FC<FormulationFormProps> = ({
  ...
  const addIngredient = () => {
    ...
    if (!Number.isFinite(percentage) || percentage <= 0) {
      toast.error('97"7"#"   97"7"#⬆  0.');
      return;
    }

    if (ingredientMap.has(itemId)) {
      toast.error('   9"97"7"#⬆9 ".');
      return;
    }

    const nextTotal = totalPercentage + percentage;
    if (nextTotal > 100.001) {
      toast.error(`9 9" 9" 97"7"#⬆  9 100% (9"9" ${totalPercentage.toFixed(2)}%).`);
      return;
    }
  ...
  const handleSave = async (event: React.FormEvent) => {
    ...
    if (!draft.ingredients.length) {
      toast.error(' 97"7"#⬆9 9 9" 9""97"7"#".');
      return;
    }
    if (Math.abs(totalPercentage - 100) > 0.001) {
      toast.error(`9 9"   9 100% (9"9" ${totalPercentage.toFixed(2)}%).`);
      return;
    }
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-4 p-5 md:grid-cols-2">
            <label className="space-y-1">
              <span className="text-xs font-semibold text-slate-600 dark:text-slate-300"> 9"#97"7"#⬆ *</span>
              <input
                value={draft.name}
            ...
            <label className="space-y-1">
              <span className="text-xs font-semibold text-slate-600 dark:text-slate-300">97"7"#⬆9 9"#97"7"#⬆</span>
              <input
                value={draft.code}
            ...
            <label className="space-y-1">
              <span className="text-xs font-semibold text-slate-600 dark:text-slate-300">9" 9" *</span>
              <select
            ...
            <label className="space-y-1">
              <span className="text-xs font-semibold text-slate-600 dark:text-slate-300"> 9" () *</span>
              <input
                type="number"
                min={0.001}
                step="0.001"
          ...
          <div className="border-t border-slate-200 px-5 py-4 dark:border-slate-700">
            <h3 className="mb-3 text-sm font-bold text-slate-900 dark:text-slate-100">97"7"#⬆9 9"#97"7"#⬆</h3>
            ...
            <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700">
              <table className="min-w-full text-sm">
                <thead className="bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200">
                  <tr>
                    <th className="px-3 py-2 text-right">9"97"7"#⬆9</th>
                    <th className="px-3 py-2 text-right">9" %</th>
                    <th className="px-3 py-2 text-right">97"7"#⬆/</th>
                    <th className="px-3 py-2 text-right">9"#97"7"#⬆7" 9" (97"7"#⬆)</th>
...
"</th>
                ...
                <tfoot className="bg-slate-50 dark:bg-slate-800/50">
                  <tr>
                    <td className="px-3 py-2 font-bold text-slate-900 dark:text-slate-100">9"9"</td>
          ...
          <div className="sticky bottom-0 z-20 flex items-center justify-between border-t border-slate-200 bg-white/95 px-5 py-4 backdrop-blur dark:border-slate-700 dark:bg-slate-900/95">
            <p className={`text-xs ${canSave ? 'text-emerald-700 dark:text-emerald-300' : 'text-amber-700 dark:text-amber-300'}`}>
              {canSave
                ? '9"#97"7"#⬆  9"9".'
                : '""   9"#97"7"#⬆97"7"#⬆7" 9" 9"97"7"#⬆7" 99 9" (100%).'}
            </p>
