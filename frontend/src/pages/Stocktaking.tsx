...
const StocktakingPage: React.FC = () => {
  ...
  const onCreateEntry = async (event: React.FormEvent) => {
    ...
    if (!canCreate) return toast.error('9 99"#
" 9  .');
    if (!form.itemId.trim()) return toast.error(' 99 "#997"7"7".');
    if (!Number.isFinite(countedStock)) return toast.error('97"7"#"97"7"#99  9 .');

    const item = itemMap.get(String(form.itemId));
    if (!item) return toast.error('99  9"".');
...
"   9</span>
        ...
        <p className="text-sm"> 9 <code>stocktaking.view</code> " <code>inventory.view.stock</code>.</p>
      </div>
    );
  }
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              97"7"#"9 99 9  997"7"#9 997"7"#99 997"7"#99  9997"7"7".
            </p>
          </div>
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            ...
            {!loading && !error && filteredRows.length === 0 && (
              <tr>
                <td colSpan={8} className="px-4 py-8 text-center text-slate-500">9 " 9 997"7"#9.</td>
              </tr>
            )}
          ...
          >
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4 dark:border-slate-700">
                <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">  </h2>
                <button
                  type="button"
